...
public class JDBean
{
  private final String sword = "Sword", spear = "Spear", axe = "Axe", bow = "Bow", staff = "Staff", dagger = "Dagger", whip = "Whip",
    other = "Cannot be used by player characters", any = "Any", red = "Coup de Grâce (Red)", purple = "Innate skill (Purple)",
    green = "Magic (Green)", blue = "Statistic (Blue)", n = "No", y = "Yes";
  
  private String[] skillNames = {"Follow Me!", "Follow Me! II", "Jugulaire", "Blood Sword", "Blood Sword II", "Flash Strike",
  ...
  private String[] skillDescriptions = {"A mighty cry to stir up comrades. Raises the attack of allies within a 4-square radius for several turns.",
  ...
  private final int followMe = 0, followMeII = 1, jugulaire = 2, bloodSword = 3, bloodSwordII = 4, flashStrike = 5, tripleSlash = 6, heavensGate = 7,
  ...
  private String[][] skillProperties = {/*followme*/{"Attack +10", red, sword, null, null, "  X\n XUX\n  X"},
  ...
  private  int[][] skillStats = {/*followme*/{2, 5, 20, 2, self}, /*followMeII*/{4, 25, 30, 2, self},
  ...
  private int[][] formulae = {{accuracyPlusTen, accuracyPlusFive, accuracyPlusFive},{adroit, counter, evadePlusTen},{adroit, preemptive, expBonus},
  ...
  private ArrayList<SkillNameNumber> stoneList;
  private String requestedStone;
  private int stone;
  
  public JDBean()
  {
    // alphabetize list of skill stone names for display on main page
    this.stoneList = new ArrayList<SkillNameNumber>(174);
    for(int i = 0; i < 174; i++)
    {
      this.stoneList.add(new SkillNameNumber(i, skillNames[i]));
    }// for
    Collections.sort(this.stoneList);
    
    // set reqStone to -1 which will tell the jsp that no stone has been requested and to show the main page
    this.stone = -1;
  }// ctor
  
  public void setRequestedStone(String rS)
  {
    // make sure the user's not being a prick and manually editing the url
    try
    {
      this.stone = Integer.parseInt(rS);// get int from url
    }// try
    catch(NumberFormatException e)
    {
      this.stone = 1000;// set to >173 so the jsp will tell the user to fuck off
    }// catch
  }// setRequestedStone
  ...
  /**
   * returns alphabetized list of skill stone names. will only be called when stone == -1.
   */
  public ArrayList<SkillNameNumber> getStoneList()
  {
    return this.stoneList;
  }// getStoneList
  
  /**
   * returns info on requested stone.  will only be called when stone >= 0.
   */
  public SkillStoneData getStoneInfo()
  {
    SkillStoneData stoneData = new SkillStoneData();

    stoneData.setName(skillNames[this.stone]);
    stoneData.setType(skillProperties[this.stone][1]);
    stoneData.setRarity(skillStats[this.stone][0]);
    stoneData.setEffect(skillProperties[this.stone][0]);
    stoneData.setRequiredLevel(skillStats[this.stone][1]);
    stoneData.setMpCost(skillStats[this.stone][2]);
    stoneData.setWeapon(skillProperties[this.stone][2]);
    stoneData.setDuration(skillStats[this.stone][3]);
    stoneData.setRange(skillStats[this.stone][4]);
    stoneData.setThruObstacles(skillProperties[this.stone][3]);
    stoneData.setReversible(skillProperties[this.stone][4]);
    stoneData.setAoe(skillProperties[this.stone][5]);
    stoneData.setDescription(skillDescriptions[this.stone]);
    
    // get skillbinding forumlae
    for(int[] formula : formulae)
    {
      // make the list of formulae that create the requested stone
      if(formula[0] == this.stone)
      {
	//stoneData.addCreationFormula(skillNames[formula[0]] + " = " + skillNames[formula[1]] + " + " + skillNames[formula[2]]);
	stoneData.addCreationFormula(new Formula(new SkillNameNumber(formula[0], skillNames[formula[0]]), new SkillNameNumber(formula[1], skillNames[formula[1]]), new SkillNameNumber(formula[2], skillNames[formula[2]])));
      }// if

      // make the list of formulae where the requested stone can be used as a component
      if(formula[1] == this.stone)
      {
	//stoneData.addComponentFormula(skillNames[formula[1]] + " + " + skillNames[formula[2]] + " = " + skillNames[formula[0]]);

	stoneData.addComponentFormula(new Formula(new SkillNameNumber(formula[1], skillNames[formula[1]]), new SkillNameNumber(formula[2], skillNames[formula[2]]), new SkillNameNumber(formula[0], skillNames[formula[0]])));
      }// if

      if(formula[2] == this.stone && formula[1] != formula[2])
      {
	//stoneData.addComponentFormula(skillNames[formula[2]] + " + " + skillNames[formula[1]] + " = " + skillNames[formula[0]]);

	stoneData.addComponentFormula(new Formula(new SkillNameNumber(formula[2], skillNames[formula[2]]), new SkillNameNumber(formula[1], skillNames[formula[1]]), new SkillNameNumber(formula[0], skillNames[formula[0]])));
      }// if
    }// for
